...
%        imwrite(uint8(normalize(smth(angle(u_rec),10))),[root_to,'\PhFB_num=',num2str(num),'_iter=',num2str(iii),'.bmp']);
       imwrite(uint8(normalize( crp(abs(u_rec),[C/2 R/2],600))),[root_to,'\Amp_num=',num2str(num),'_iter=',num2str(iii),'.bmp']);
       imwrite(uint8(normalize(crp(smth(angle(u_rec),10),[C/2 R/2],600))),[root_to,'\Ph_num=',num2str(num),'_iter=',num2str(iii),'.bmp']);
    end
end
tFB = toc;
end
function [u_rec,tFB,mse_amp_fb,mse_ph_fb] = sbmir_fc(lambda,cp,dist,z0,num,iter,amps,gph,root_to,method)
...
for iii =1:iter
    ...
    for ii = 1:num-1
        ...
        forward = prop(a.*exp(1i*phase),lambda,-(z0+(ii-1)*dist),cp,method);
        forward = prop(forward,lambda,(z0+(ii)*dist),cp,method);
        phase = angle(forward);
    end  
    for ii = 1:num-1
       a = amps(:,:,num+1-ii);
       ...
       backward = prop(a.*exp(1i*phase),lambda,-(z0+(num-ii)*dist),cp,method);
       backward = prop(backward,lambda,z0+dist*(num-ii-1),cp,method);
